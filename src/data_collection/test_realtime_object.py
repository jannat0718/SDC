...
# change lidar_NAGEL_OFFSET=0 for the lidar in the lab
# change lidar_NAGEL_OFFSET=180 for the lidar on the kart (cable side = 180° by hardware convention)
# --- Configuration ---
...
NMS_THRESHOLD  = 0.40     # overlap IOU threshold for NMS
...
class ObjectDetector:
    ...
    def _decode_end2end(self, raw: np.ndarray, img_w: int, img_h: int):
        """Output format: [1, N, 6]  →  x1,y1,x2,y2,score,class_id (scaled to model input)."""
...
class LidarStream:
    ...
    # 180° so vehicle-forward = +X. Verified from 23-04-2026 bench test
    # (car at 0.75m in front → returns clustered at raw lidar angle ≈ 190°).
    ...
    try:
        while True:
            ...
            mini = lidar.get_minimap(
                ...
                hfov_deg=70.0    # ← change to your camera's real horizontal FOV
            )
