...
class XboxCanController:
    ...
    def start(self) -> None:
        ...
        self.steer_task = self.bus.send_periodic(steer_msg, self.sending_speed)
        self.motor_task = self.bus.send_periodic(motor_msg, self.sending_speed)
    ...
    def step(self, timeout_s: float = 0.001) -> Dict[str, object]:
        ...
        if not ready:
            feedback = decode_feedback(self.can_listener.get_values()) if self.can_listener else {}
            return {"state": self.state, "feedback": feedback}
        ...
        if type_ & EVENT_BUTTON and value == 1:
            if number == BTN_B_KILL:
                ...
                if self.motor_task:
                    stop_msg = can.Message(arbitration_id=0x330, data=[0, 0, 0, 0, 0, 0, 0, 0], is_extended_id=False)
                    self.motor_task.modify_data(stop_msg)
            elif number == BTN_X_RECORD:
                self.state.camera_active = True
                print("[Controller] Camera ON (X pressed)")
            elif number == BTN_Y_STOP_REC:
                self.state.camera_active = False
                print("[Controller] Camera OFF (Y pressed)")
            ...
            if number == AXIS_LEFT_X and self.steer_task:
                ...
                steer_msg = can.Message(
                    ...
                    data=list(struct.pack("<f", self.state.steering_command)) + [0] * 4,
                    is_extended_id=False,
                )
                self.steer_task.modify_data(steer_msg)

            elif number == AXIS_RIGHT_Y and self.motor_task:
                self.state.throttle_command = round(-(value / 32767.0), 4)
                speed = int(abs(self.state.throttle_command) * 45) if abs(value) > 5000 else 0
                gear = 1 if self.state.throttle_command > 0.15 else (2 if self.state.throttle_command < -0.15 else 0)
                motor_msg = can.Message(
                    arbitration_id=0x330,
                    data=[speed, 0, gear, 0, 0, 0, 0, 0],
                    is_extended_id=False,
                )
                self.motor_task.modify_data(motor_msg)
    ...
    def stop(self) -> None:
        ...
        if self.motor_task:
            self.motor_task.stop()
        if self.can_listener:
